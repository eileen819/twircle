...
- 특히 **Firebase Firestore 실시간 구독(`onSnapshot`)을 활용**하여 타임라인, 댓글, 알림이 즉시 반영되도록 구현했으며, **Firebase Auth 기반으로 로그인/회원가입을 구현**하고, Context API로 인증/팔로우/언어 상태를 전역 관리해 불필요한 리렌더링을 최소화했습니다. 이 외에도 **해시태그 기반 검색을 지원**합니다.
